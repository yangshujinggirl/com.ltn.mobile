...
export function openApp(browserName, showConfirmModal, schemeUrl) {
  ...
  if (browserName === 'Chrome') {
    ...
    if (platforms.isAndroid) {
      showConfirmModal(() => {
        setTimeout(() => {
          ...
          `
          http://zhushou.360.cn/detail/index/soft_id/3198443
          `;
        }, 3000);
      });
    } else {
    ...
    if (platforms.isAndroid) {
      window.location.href = 'http://zhushou.360.cn/detail/index/soft_id/3198443';
    } else {
      window.location.href = 'http://a.app.qq.com/o/simple.jsp?pkgname=com.wy.lingtouniao';
    }
...
//银行存管弹框
export function openCunGuan(url) {
  window.LtnApp.openCunGuan({
    url
  });
}
