...
const BankLimitList=({data})=>(
  <div className="tabel-wrap">
    <p className="table-tr">
      <span className="table-td">银行</span>
      <span className="table-td">单笔限额<br/>（万）</span>
      <span className="table-td">单日限额<br/>（万）</span>
    </p>
    {
      data.map((el,index)=>(
        <p className="table-tr" key={index}>
          <span className="table-td">{el.bankName}</span>
          <span className="table-td">{el.chargeTimeLimit}</span>
          <span className="table-td">{el.chargeDateLimit}</span>
        </p>
      ))
    }
  </div>
...
class FaqRechargePage extends React.Component{
  constructor(props) {
    super(props);
    this.state={
      bankList:[]
  ...
  getBankLimitList() {
    ApiClient.getBankLimitApi()
    .then((data) => {
      this.setState({
        bankList: data.data.bankInfoList
      })
      console.log(data)
    }, (error) =>{
        ...
        <Accordion defaultActiveKey={type}>
          <Accordion.Panel header="一、充值是否有上限？" >
            <div className="faq-text-content">
              ...
              {
                this.state.bankList&&<BankLimitList data={this.state.bankList}/>
              }
            </div>
          </Accordion.Panel>
